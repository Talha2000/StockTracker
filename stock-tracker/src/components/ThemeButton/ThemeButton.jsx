  ...
  return (
    <div>
      <button
        ...
        className={`relative inline-flex items-center px-4 py-2 border border-transparent border-slate-100 shadow-sm text-sm font-medium rounded-md text-white focus:outline-none focus:ring-2 focus:ring-offset-2
        ${darkMode ? "border-slate-500" : ""}`}
      >
        {/* darkMode ? <MoonIcon className={moonIconClasses} aria-hidden="true"/> */}
        {darkMode ? (
          <MoonIcon className={moonIconClasses} aria-hidden="true" />
        ) : (
          <SunIcon className={moonIconClasses} aria-hidden="true" />
        )}
      </button>
    </div>
...
{/* <label className="relative flex items-center justify-center cursor-pointer ">
  ...
  <span className="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300"><MoonIcon className={moonIconClasses} /></span>
</label> */}